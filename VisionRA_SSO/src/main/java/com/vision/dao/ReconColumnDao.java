...
@Component
public class ReconColumnDao extends AbstractDao<ReconHeaderVb> {

	public List<ReconHeaderVb> getQueryReconColumns(ReconHeaderVb dObj, int intStatus) {
		...
		strQueryAppr = new String("SELECT TAPPR.RULE_ID,                                "
				...
				+ "  	  FROM RA_RECON_COLUMNS_REPOSITORY  t4  left join (select * from RA_RECON_COLUMNS a1 where rule_id = ?) TAPPR "
		...
		strQueryPend = new String("SELECT TPEND.RULE_ID,                                "
				...
				+ "  	  FROM RA_RECON_COLUMNS_REPOSITORY  t4  left join (select * from RA_RECON_COLUMNS_PEND a1 where rule_id = ?) TPEND "
...
*/			return null;
	...
	protected int doInsertionApprReconColumn(ReconHeaderVb vObject) {
		String query = " INSERT INTO RA_RECON_COLUMNS (RULE_ID_AT,RULE_ID,TABLE_ID,COL_ID,COL_TYPE,COL_NAME,ALIAS,                       "
	...
	protected int doInsertionPendReconColumn(ReconHeaderVb vObject) {
		String query = " INSERT INTO RA_RECON_COLUMNS_PEND (RULE_ID_AT,RULE_ID,TABLE_ID,COL_ID,COL_TYPE,COL_NAME,ALIAS,                       "
				+ " AGG_FUNCTION,GROUP_BY,STATUS_NT,STATUS,RECORD_INDICATOR_NT,                        "
				+ " RECORD_INDICATOR,MAKER,VERIFIER,INTERNAL_STATUS,DATE_LAST_MODIFIED,DATE_CREATION,ORDER_BY)  "
				+ " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,   " + getDbFunction("SYSDATE")
				+ "," + getDbFunction("SYSDATE") + ",?)";
	...
	protected int doInsertionReconColumnHis(ReconHeaderVb vObject) {
		String query = " INSERT INTO RA_RECON_COLUMNS_HIS (REF_NO,RULE_ID_AT,RULE_ID,TABLE_ID,COL_ID,COL_TYPE        "
	...
	protected int deleteApprReconColumn(ReconHeaderVb vObject) {
		String query = "DELETE FROM RA_RECON_COLUMNS WHERE RULE_ID = ?    ";
		Object[] args = { vObject.getRuleId() };
		return getJdbcTemplate().update(query, args);
	}

	protected int deletePendReconColumn(ReconHeaderVb vObject) {
		String query = "DELETE FROM RA_RECON_COLUMNS_PEND WHERE RULE_ID = ?   ";
		Object[] args = { vObject.getRuleId() };
		return getJdbcTemplate().update(query, args);
	}

	public ExceptionCode deleteAndInsertApprReconColumn(ReconHeaderVb vObject) {
		ExceptionCode exceptionCode = new ExceptionCode();
		List<ReconHeaderVb> collTemp = getQueryReconColumns(vObject, Constants.STATUS_ZERO);
		...
		List<ReconHeaderVb> selectionCriteriaLst = vObject.getSelectionCriteriaLst();
		for (ReconHeaderVb reconHeaderVb : selectionCriteriaLst) {
			reconHeaderVb.setMaker(vObject.getMaker());
			reconHeaderVb.setRecordIndicator(vObject.getRecordIndicator());
			retVal = doInsertionApprReconColumn(reconHeaderVb);
	...
	public ExceptionCode deleteAndInsertPendReconColumn(ReconHeaderVb vObject) {
		ExceptionCode exceptionCode = new ExceptionCode();
		List<ReconHeaderVb> collTemp = getQueryReconColumns(vObject, Constants.SUCCESSFUL_OPERATION);
		if (collTemp != null && collTemp.size() > 0) {
		...
		for (ReconHeaderVb reconHeaderVb : selectionCriteriaLst) {
			...
			reconHeaderVb.setMaker(vObject.getMaker());
			reconHeaderVb.setRecordIndicator(vObject.getRecordIndicator());
			retVal = doInsertionPendReconColumn(reconHeaderVb);
			writeAuditLog(reconHeaderVb, null);
			if (retVal != Constants.SUCCESSFUL_OPERATION){
				exceptionCode = getResultObject(retVal);
				throw buildRuntimeCustomException(exceptionCode);
	...
	public List<ReconHeaderVb> getQueryReconColumnsRepository() {
		List<ReconHeaderVb> collTemp = null;
		setServiceDefaults();
		String strQueryAppr = null;
