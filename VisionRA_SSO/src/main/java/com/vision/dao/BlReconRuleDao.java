...
@Component
public class BlReconRuleDao extends AbstractDao<BlReconRuleVb> {
	@Override
	protected void setServiceDefaults() {
		serviceName = "BusinessLineReconRule";
		serviceDesc = "Business Line Recon Rule";
		tableName = "RA_BL_RECON_RULE";
		childTableName = "RA_BL_RECON_RULE";
		intCurrentUserId = CustomContextHolder.getContext().getVisionId();
	}

	public List<BlReconRuleVb> getBlReconRuleDetails(BusinessLineHeaderVb vObject, int intStatus) {
		...
		try {
			...
			if (intStatus == Constants.STATUS_ZERO) {
				query = " SELECT TAPPR.COUNTRY,                                             "
						...
						+ "        TAPPR.DATE_CREATION                                        "
						+ "   FROM RA_BL_RECON_RULE TAPPR                                  "
						+ "  WHERE TAPPR.COUNTRY = ? AND TAPPR.LE_BOOK = ? AND TAPPR.BUSINESS_LINE_ID = ? ";
			} else {
				query = " SELECT TPEND.COUNTRY,                                             "
						...
						+ "        TPEND.DATE_CREATION                                        "
						+ "   FROM RA_BL_RECON_RULE_PEND TPEND                                  "
						+ "  WHERE TPEND.COUNTRY = ? AND TPEND.LE_BOOK = ? AND TPEND.BUSINESS_LINE_ID = ? ";
			}
	...
	protected int doInsertApprBlReconRule(BlReconRuleVb vObject) {
		String query = " INSERT INTO RA_BL_RECON_RULE (COUNTRY,LE_BOOK,BUSINESS_LINE_ID, "
				+ "  RECON_TYPE_AT,RECON_TYPE,                                     "
		...
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getBusinessLineId(),
				vObject.getReconTypeAT(), vObject.getReconType(), vObject.getReconMethodAT(), vObject.getReconMethod(),
	...
	protected int doInsertPendBlReconRule(BlReconRuleVb vObject) {
		String query = " INSERT INTO RA_BL_RECON_RULE_PEND (COUNTRY,LE_BOOK,BUSINESS_LINE_ID, "
				+ "  RECON_TYPE_AT,RECON_TYPE,                                     "
				+ "  RECON_METHOD_AT,RECON_METHOD,                                 "
				+ "  RULE_ID_AT,RULE_ID,                                           "
				+ "  RECON_GRACE_DAYS,RULE_PRIORITY,                               "
	...
	protected int doInsertBlReconRuleHis(BlReconRuleVb vObject) {
		String query = " INSERT INTO RA_BL_RECON_RULE_HIS (REF_NO,COUNTRY,LE_BOOK,BUSINESS_LINE_ID         "
				+ " ,RECON_TYPE_AT,RECON_TYPE,RECON_METHOD_AT,RECON_METHOD,RULE_ID_AT,RULE_ID         "
				+ " ,RECON_GRACE_DAYS,RULE_PRIORITY,STATUS_NT,STATUS,RECORD_INDICATOR_NT              "
	...
	protected int doDeleteApprBlReconRule(BusinessLineHeaderVb vObject) {
		String query = " DELETE FROM RA_BL_RECON_RULE WHERE COUNTRY = ? AND LE_BOOK = ?  "
				+ " AND BUSINESS_LINE_ID = ?     ";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getBusinessLineId() };
		return getJdbcTemplate().update(query, args);
	}

	protected int doDeletePendBlReconRule(BusinessLineHeaderVb vObject) {
		String query = " DELETE FROM RA_BL_RECON_RULE_PEND WHERE COUNTRY = ? AND LE_BOOK = ?  "
				+ " AND BUSINESS_LINE_ID = ?     ";
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getBusinessLineId() };
		return getJdbcTemplate().update(query, args);
	}

	public ExceptionCode deleteAndInsertApprBlReconRule(BusinessLineHeaderVb vObject) throws RuntimeCustomException {
		ExceptionCode exceptionCode = null;
		ArrayList<BlReconRuleVb> blReconRulelst = (ArrayList<BlReconRuleVb>) vObject.getBlReconRulelst();
		List<BlReconRuleVb> collTemp = null;
		collTemp = getBlReconRuleDetails(vObject, Constants.STATUS_ZERO);
		if (collTemp != null && collTemp.size() > 0) {
			collTemp.forEach(blReconRule ->{
				blReconRule.setRefNo(getMaxSequence());
				doInsertBlReconRuleHis(blReconRule);
			});
			int delCnt = doDeleteApprBlReconRule(vObject);
		}
		for (BlReconRuleVb blReconRuleVb : blReconRulelst) {
			...
			retVal = doInsertApprBlReconRule(blReconRuleVb);
	...
	public ExceptionCode deleteAndInsertPendBlReconRule(BusinessLineHeaderVb vObject) throws RuntimeCustomException {
		ExceptionCode exceptionCode = null;
		ArrayList<BlReconRuleVb> blReconRulelst = (ArrayList<BlReconRuleVb>) vObject.getBlReconRulelst();
		List<BlReconRuleVb> collTemp = null;
		collTemp = getBlReconRuleDetails(vObject, Constants.SUCCESSFUL_OPERATION);
		if (collTemp != null && collTemp.size() > 0) {
			int delCnt = doDeletePendBlReconRule(vObject);
		}
		for (BlReconRuleVb blReconRuleVb : blReconRulelst) {
			blReconRuleVb.setCountry(vObject.getCountry());
			blReconRuleVb.setLeBook(vObject.getLeBook());
			...
			blReconRuleVb.setVerifier(intCurrentUserId);
			// blReconRuleVb.setReconStatus(vObject.getBusinessLineStatus());
			retVal = doInsertPendBlReconRule(blReconRuleVb);
			if (retVal != Constants.SUCCESSFUL_OPERATION) {
				exceptionCode = getResultObject(retVal);
				throw buildRuntimeCustomException(exceptionCode);
			} else {
	...
	public int getMaxSequence(){
		StringBuffer strBufApprove = new StringBuffer("Select MAX(TAppr.REF_NO) From RA_BL_RECON_RULE_HIS TAppr ");
		try{
