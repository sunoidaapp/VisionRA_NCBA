...
@Component
public class TaxReconActivityFilterDao extends AbstractDao<TaxReconActivityFilterVb> {
	...
	protected void setServiceDefaults() {
		...
		childTableName = "RA_RECON_ACT_FILTER_TAX";
		intCurrentUserId = CustomContextHolder.getContext().getVisionId();
	}

	protected int doInsertionApprReconActFilter(TaxReconActivityFilterVb vObject) {
		try {
			String query = "INSERT INTO RA_RECON_ACT_FILTER_TAX(RULE_ID,FILTER_SEQUENCE,LHS_FILTER_TABLE,LHS_FILTER_COLUMN,CONDITION_OPERATION, "
	...
	protected int doInsertionReconActFilterHis(TaxReconActivityFilterVb vObject) {
		try {
			String query =" INSERT INTO RA_RECON_ACT_FILTER_TAX_HIS(REF_NO,RULE_ID,FILTER_SEQUENCE "
	...
	protected int doInsertionPendReconActFilter(TaxReconActivityFilterVb vObject) {
		String query = "INSERT INTO RA_RECON_ACT_FILTER_TAX_PEND(RULE_ID,FILTER_SEQUENCE,LHS_FILTER_TABLE,LHS_FILTER_COLUMN,CONDITION_OPERATION, "
				+ "CONDITION_TYPE,RHS_FILTER_TABLE,RHS_FILTER_COLUMN,CONDITION_VALUE1,CONDITION_VALUE2,FILTER_STATUS_NT,FILTER_STATUS,"
				+ "RECORD_INDICATOR_NT,RECORD_INDICATOR,MAKER,VERIFIER,DATE_LAST_MODIFIED,DATE_CREATION,LHS_FILTER_COL_TYPE,RHS_FILTER_COL_TYPE)                              "
				+ "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " + getDbFunction("SYSDATE") + ","
				+ getDbFunction("SYSDATE") + ",?,?)";
	...
	protected int deleteApprReconActFilter(TaxReconHeaderVb vObject) {
		String query = "DELETE FROM RA_RECON_ACT_FILTER_TAX  WHERE RULE_ID = ?  ";

		Object[] args = { vObject.getRuleId() };
		return getJdbcTemplate().update(query, args);
	}

	protected int deletePendReconActFilter(TaxReconHeaderVb vObject) {
		String query = "DELETE FROM RA_RECON_ACT_FILTER_TAX_PEND WHERE RULE_ID = ?  ";

		Object[] args = { vObject.getRuleId() };
		return getJdbcTemplate().update(query, args);
	}
	public List<TaxReconActivityFilterVb> getQueryReconActFilter(TaxReconHeaderVb dObj, int intStatus) {
		...
		strQueryAppr = new String(" SELECT TAPPR.RULE_ID,                    "
				...
				+ "  FROM RA_RECON_ACT_FILTER_TAX TAPPR,NUM_SUB_TAB T2, NUM_SUB_TAB T3 "
		...
		strQueryPend = new String(" SELECT TPEND.RULE_ID,                    "
				...
				+ "  FROM RA_RECON_ACT_FILTER_TAX_PEND TPEND, NUM_SUB_TAB T2, NUM_SUB_TAB T3 "
...
*/			return null;
	...
	public List<TaxReconActivityFilterVb> getQueryReconActFilterRec(TaxReconActivityFilterVb dObj, int intStatus) {
		List<TaxReconActivityFilterVb> collTemp = null;
		setServiceDefaults();
		String strQueryAppr = null;
		String strQueryPend = null;
...
*/			return null;
	...
	public List<TaxReconActivityFilterVb> getQueryReconRelationFilter(TaxReconHeaderVb dObj, int intStatus,String conditionType) {
		List<TaxReconActivityFilterVb> collTemp = null;
		setServiceDefaults();
		String strQueryAppr = null;
		String strQueryPend = null;
	...
	protected RowMapper getReconActFilterMapper() {
		RowMapper mapper = new RowMapper() {
			...
			public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
				TaxReconActivityFilterVb TaxReconActivityFilterVb = new TaxReconActivityFilterVb();
				TaxReconActivityFilterVb.setRuleId(rs.getString("RULE_ID"));
				TaxReconActivityFilterVb.setFilterSequence(rs.getInt("FILTER_SEQUENCE"));
				TaxReconActivityFilterVb.setLhsFilterTable(rs.getString("LHS_FILTER_TABLE"));
				TaxReconActivityFilterVb.setLhsFilterColumn(rs.getString("LHS_FILTER_COLUMN"));
				TaxReconActivityFilterVb.setConditionOperation(rs.getString("CONDITION_OPERATION"));
				TaxReconActivityFilterVb.setConditionType(rs.getString("CONDITION_TYPE"));
				TaxReconActivityFilterVb.setRhsFilterColumn(rs.getString("RHS_FILTER_COLUMN"));
				TaxReconActivityFilterVb.setRhsFilterTable(rs.getString("RHS_FILTER_TABLE"));
				TaxReconActivityFilterVb.setConditionValue1(rs.getString("CONDITION_VALUE1"));
				TaxReconActivityFilterVb.setConditionValue2(rs.getString("CONDITION_VALUE2"));
				TaxReconActivityFilterVb.setFilterStatus(rs.getInt("FILTER_STATUS"));
				TaxReconActivityFilterVb.setRecordIndicator(rs.getInt("RECORD_INDICATOR"));
				TaxReconActivityFilterVb.setMaker(rs.getInt("MAKER"));
				TaxReconActivityFilterVb.setVerifier(rs.getInt("VERIFIER"));
				TaxReconActivityFilterVb.setDateLastModified(rs.getString("DATE_LAST_MODIFIED"));
				TaxReconActivityFilterVb.setDateCreation(rs.getString("DATE_CREATION"));
				if("FILTER".equalsIgnoreCase(TaxReconActivityFilterVb.getConditionType())) {
					TaxReconActivityFilterVb.setFilterTableId(rs.getString("LHS_FILTER_TABLE"));
				}
				TaxReconActivityFilterVb.setLhsFilterColType(rs.getString("LHS_FILTER_COL_TYPE"));
				TaxReconActivityFilterVb.setRhsFilterColType(rs.getString("RHS_FILTER_COL_TYPE"));
				return TaxReconActivityFilterVb;
			}
		};
		return mapper;
	...
	public ExceptionCode deleteAndInsertApprRelationFilter(TaxReconHeaderVb vObject) {
		ExceptionCode exceptionCode = new ExceptionCode();
		List<TaxReconActivityFilterVb> collTemp = getQueryReconActFilter(vObject, Constants.STATUS_ZERO);
		List<TaxReconActivityFilterVb> collTempTab=taxReconTabRelationDao.getQueryReconTabRelationRuleid(vObject, Constants.STATUS_ZERO);
		int delCnt = 0;
		...
		if(collTempTab != null && collTempTab.size() > 0) {
			...
			delCnt = taxReconTabRelationDao.deleteApprReconTabRelation(vObject);
		}
			
		ArrayList<TaxReconActivityFilterVb> tableRellst = new ArrayList<TaxReconActivityFilterVb>();
		Boolean filterCondition = false;
		if(vObject.getRelationJoinsLst() != null && vObject.getRelationJoinsLst().size() > 0 ) {
			//Relation Joins
			for (TaxReconActivityFilterVb reconRelationVb : vObject.getRelationJoinsLst()) {
				...
				if (reconRelationVb.getJoinsLst() != null && reconRelationVb.getJoinsLst().size() > 0) {
					List<TaxReconActivityFilterVb> joinsLst = reconRelationVb.getJoinsLst();
					for (TaxReconActivityFilterVb joinVb : joinsLst) {
				...
				TaxReconActivityFilterVb filterVb = new TaxReconActivityFilterVb();
				...
				if(exceptionCode.getErrorCode() != Constants.ERRONEOUS_OPERATION) {
					tableRellst = (ArrayList<TaxReconActivityFilterVb>)exceptionCode.getRequest();
				}
			}
		}
		if(tableRellst != null && tableRellst.size() > 0) {
			for(TaxReconActivityFilterVb tabRelVb : tableRellst) {
				...
				retVal = taxReconTabRelationDao.doInsertionApprReconTabRelation(tabRelVb);
			}
		}
		//Relation Filters
		if (vObject.getFiltersLst() != null && vObject.getFiltersLst().size() > 0) {
			for (TaxReconActivityFilterVb reconFilterVb : vObject.getFiltersLst()) {
		...
		/*if(ValidationUtil.isValid(vObject.getJoinString2())|| ValidationUtil.isValid(vObject.getFilterConditon1())) {
			TaxReconActivityFilterVb reconTabFilter = new TaxReconActivityFilterVb();
			...
			retVal = taxReconTabRelationDao.doInsertionApprReconTabRelation(reconTabFilter);
		}*/
		exceptionCode = getResultObject(Constants.SUCCESSFUL_OPERATION);
		return exceptionCode;
	}

	public ExceptionCode deleteAndInsertPendRelationFilter(TaxReconHeaderVb vObject) {
		ExceptionCode exceptionCode = new ExceptionCode();
		List<TaxReconActivityFilterVb> collTemp = getQueryReconActFilter(vObject, Constants.SUCCESSFUL_OPERATION);
		if (collTemp != null && collTemp.size() > 0) {
			int delCnt = deletePendReconActFilter(vObject);
			delCnt = taxReconTabRelationDao.deletePendReconTabRelation(vObject);
		}
		ArrayList<TaxReconActivityFilterVb> tableRellst = new ArrayList<TaxReconActivityFilterVb>();
		Boolean filterCondition = false;
		...
		for(TaxReconActivityFilterVb tabRelVb : tableRellst) {
			...
			tabRelVb.setFromTableId(Integer.parseInt(tabRelVb.getLhsFilterTable()));
			tabRelVb.setToTableId(Integer.parseInt(tabRelVb.getRhsFilterTable()));
			retVal = taxReconTabRelationDao.doInsertionPendReconTabRelation(tabRelVb);
		}
		...
		/*if(ValidationUtil.isValid(vObject.getJoinString2())|| ValidationUtil.isValid(vObject.getFilterConditon1())) {
			...
			reconTabFilter.setMaker(vObject.getMaker());
			reconTabFilter.setVerifier(vObject.getVerifier());
			retVal = taxReconTabRelationDao.doInsertionPendReconTabRelation(reconTabFilter);
		}*/
	...
	public ExceptionCode getTableRelationsList(ArrayList<TaxReconActivityFilterVb> tableRellst,TaxReconActivityFilterVb tableRelations) {
		ExceptionCode exceptionCode = new ExceptionCode();
		try {
